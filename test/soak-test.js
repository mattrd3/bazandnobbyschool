...
function assertExpectedDay(dateKey, day){ const e=expected.get(dateKey); if(!e) throw new Error(`missing expected state for ${dateKey}`); if(e.removed) return; const got=new Set((day.players||[]).filter(p=>p.startsWith(RUN_ID))); assertNoDuplicates(day.players||[]); assert.equal("maybes" in day, false, "maybes should not exist in v15 data"); assert.deepEqual([...got].sort(), [...e.players].sort(), `players mismatch for ${dateKey}`); }
...
async function opTogglePublic(){ const openDates=TEST_DATES.filter(d=>!expected.get(d).locked); if(!openDates.length){ inc("public skip all locked"); return; } const dateKey=pick(openDates); const player=pick(PLAYERS); const e=expected.get(dateKey); const before=e.players.has(player); const status=before?"none":"playing"; const r=await post("/api/player-status", {dateKey,name:player,status,playerName:player,playerPin:PIN,competition:e.competition||"Soak Test"}); assert.equal(r.body.ok,true); before ? e.players.delete(player) : e.players.add(player); inc(status==="playing"?"public add":"public remove"); }
