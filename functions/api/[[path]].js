...
function initDay(competition = "") { return { players: [], priorityPlayers: [], locked: false, competition, audit: [], draw: null }; }
function safeDay(raw, competition = "") {
  ...
  return { ...base, ...rest, players, priorityPlayers, locked: Boolean(raw.locked), competition: (typeof raw.competition === "string" && raw.competition.trim()) ? raw.competition.trim() : competition, audit: Array.isArray(raw.audit) ? raw.audit : [], draw: raw.draw || null };
}
...
async function handle(request, env) {
  ...
  if (route === "player-status" && method === "POST") {
    ...
    if(!/^\d{4}-\d{2}-\d{2}$/.test(dateKey)) return json({ok:false,error:"Invalid dateKey"},400); if(!name) return json({ok:false,error:"Missing player name"},400); if(!["playing","none"].includes(status)) return json({ok:false,error:"Invalid player status"},400);
